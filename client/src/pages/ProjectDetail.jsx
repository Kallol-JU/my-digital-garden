...
export default function ProjectDetail() {
    ...
    if (loading) return <div className="animate-fade-in max-w-2xl text-gray-500 pt-12">Loading build logs...</div>;
    if (!project) return <div className="animate-fade-in max-w-2xl text-gray-900 pt-12">Project not found.</div>;
    ...
    return (
        <div className="animate-fade-in max-w-2xl">
            ...
            <article className="prose prose-gray max-w-none text-gray-800 space-y-6 leading-relaxed 
  [&>p]:mb-6 
  ...
  [&>img]:my-8 [&>img]:rounded-md [&>img]:border [&>img]:border-gray-100">
            ...
            <div className="mt-20 pt-6 border-t border-gray-100">
                <div className="flex justify-between items-center text-[14px]">
                    <div className="w-1/3">
                        {prevProject ? (
                            <Link to={`/projects/${prevProject.slug}`} className="text-blue-600 hover:underline">
                                ← {prevProject.title}
                            </Link>
                        ) : <span />}
                    </div>
                    ...
                    <div className="w-1/3 text-right">
                        {nextProject ? (
                            <Link to={`/projects/${nextProject.slug}`} className="text-blue-600 hover:underline">
                                {nextProject.title} →
                            </Link>
                        ) : <span />}
                    </div>
